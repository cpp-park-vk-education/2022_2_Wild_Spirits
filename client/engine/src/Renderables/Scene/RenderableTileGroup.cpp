...
namespace LM {
    ...
    RenderableTileGroup::RenderableTileGroup(Ref<Texture2D> texture, glm::uvec2 size)
        ...
        m_Transform.position = glm::vec2(winSize.x / 2.0f - m_Size.x * m_Transform.scale.x / 2.0f,
    ...
    bool RenderableTileGroup::hasHovered() const { return m_HoveredId < m_Tiles.size(); }

    size_t RenderableTileGroup::getHoveredX() const { return m_HoveredId % m_Size.x; }

    size_t RenderableTileGroup::getHoveredY() const { return m_HoveredId / m_Size.x; }
    ...
    void RenderableTileGroup::onEvent(Ref<Event> event) {
        ...
        dispatcher.dispatch<MouseMovedEvent>([&](Ref<MouseMovedEvent> e) {
            glm::vec2 cursorPos =
                glm::vec2(e->getX() / Application::get()->getWindow()->getWidth() * winSize.x,

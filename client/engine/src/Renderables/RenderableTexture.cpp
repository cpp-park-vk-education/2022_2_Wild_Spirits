...
namespace LM {
    ...
    RenderableTexture::RenderableTexture(const RenderableTextureProps& props)
        : Renderable(props.transform, props.color),
        ...
        Ref<VertexBuffer> verticesVBO =
            CreateRef<VertexBuffer>(vertices.data(), sizeof(vertices[0]) * vertices.size());
    ...
    void RenderableTexture::draw(RendererInterface* renderer) { renderer->draw(this); }

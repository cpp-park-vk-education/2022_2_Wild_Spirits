...
namespace LM {
    ...
    Scene::Scene() : m_Renderer(CreateScope<Renderer>()) { }

    Scene::~Scene() { }

    void Scene::add(Ref<RenderableInterface> renderable) { m_Renderables.emplace_back(renderable); }
        ...
        dispatcher.dispatch<MouseMovedEvent>([&](Ref<MouseMovedEvent> e) {
            ...
            for (auto& renderable : m_Renderables) {
                Ref<MouseMovedEvent> newEvent = CreateRef<MouseMovedEvent>(
                    e->getX(), Application::get()->getWindow()->getHeight() - e->getY());
    ...
    void Scene::render() {
        ...
        m_Renderer->start(glm::uvec2(Application::get()->getWindow()->getWidth(),

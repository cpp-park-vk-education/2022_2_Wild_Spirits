...
namespace LM {
    ...
    Renderer::Renderer() {
        ...
        m_Shader = CreateScope<Shader>(ShaderLayout({
            {  ShaderSource::Type::kVertex,
             ShaderSource::fromFile(std::string(RES_FOLDER) + "Shaders/Simple.vert")},
            {ShaderSource::Type::kFragment,
             ShaderSource::fromFile(std::string(RES_FOLDER) + "Shaders/Simple.frag")}
        }));
    }
    ...
    void Renderer::popTransform() { m_Transforms.pop_back(); }

    void Renderer::draw(RenderableTexture* renderable) {
        m_Shader->setUniformMat4("u_Transform", m_Transforms[m_Transforms.size() - 1] *
                                                    renderable->getTransform().getMatrix());
        ...
        if (renderable->getVertexArray()->getIndexBuffer()->getCount()) {
            glDrawElements(GL_TRIANGLES, renderable->getVertexArray()->getIndexBuffer()->getCount(),

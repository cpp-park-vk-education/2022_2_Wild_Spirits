...
ChangeGetter::ChangeGetter(DnD::LogicProcessor &gameState, DnD::GameMap& gameMap): handlers(), game_state(gameState),game_map(gameMap) , loading_instances() {
...
std::string ChangeGetter::getInstance(){
    ...
    npc_loader.Load(game_state, instance_object, game_map);
    activatableItemsLoader ai_loader;
    ai_loader.Load(game_state, instance_object, game_map);
    allCharactersLoader char_loader;
    char_loader.Load(game_state, instance_object, game_map);
    itemsLoader items_loader;
    items_loader.Load(game_state, instance_object, game_map);
    weaponsLoader weapons_Loader;
    weapons_Loader.Load(game_state, instance_object, game_map);
    spellsLoader spells_loader;
    spells_loader.Load(game_state, instance_object, game_map);
    armorLoader armor_loader;
    armor_loader.Load(game_state, instance_object, game_map);
